...
echo -e "${BLUE}║     CA3: Container Observability (Docker Swarm)           ║${NC}"
echo -e "${BLUE}╚═══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
[managers]
manager1 ansible_host=$(cd terraform && terraform output -raw manager_public_ip) ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/docker-swarm-key ansible_ssh_common_args='-o StrictHostKeyChecking=no' ansible_become=true
...
EOF
    ...
    if command -v jq &> /dev/null; then
        ...
        if (cd terraform && terraform output -json worker_private_ips 2>&1) | grep -q '\['; then
            ...
            for i in $(seq 0 $((WORKER_COUNT-1))); do
                ...
                echo "worker$((i+1)) ansible_host=${WORKER_IP} ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/docker-swarm-key ansible_ssh_common_args='-o StrictHostKeyChecking=no -o ProxyJump=ubuntu@${MANAGER_IP}' ansible_become=true" >> ansible/inventory.ini
            done
        fi
    fi
    ...
    echo "This takes about 3-4 minutes for cloud-init and Docker installation..."
    echo "Waiting for cloud-init to complete on manager..."
    
    # Wait for cloud-init to complete and SSH to be available
    MAX_ATTEMPTS=60
    ATTEMPT=0
    while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
        if ssh -i ~/.ssh/docker-swarm-key -o StrictHostKeyChecking=no -o ConnectTimeout=5 ubuntu@${MANAGER_IP} 'test -f /var/lib/cloud/instance/boot-finished' 2>/dev/null; then
            echo -e "${GREEN}✓ Manager cloud-init complete${NC}"
            break
        fi
        ATTEMPT=$((ATTEMPT + 1))
        echo "Waiting for cloud-init... attempt $ATTEMPT/$MAX_ATTEMPTS"
        sleep 5
    done
    
    if [ $ATTEMPT -eq $MAX_ATTEMPTS ]; then
        echo -e "${RED}WARNING: Timeout waiting for cloud-init, continuing anyway...${NC}"
    fi
    
    # Additional wait for Docker to be fully ready
    echo "Waiting for Docker to be ready on manager..."
    sleep 30
    ...
    echo -e "${YELLOW}→ Waiting for application services to stabilize...${NC}"
    sleep 30
    ...
    echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}Phase 2: Observability Stack Deployment${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo ""
    
    echo -e "${YELLOW}→ Deploying observability stack (Loki, Prometheus, Grafana)...${NC}"
    ssh -i ~/.ssh/docker-swarm-key ubuntu@${MANAGER_IP} 'cd ~/plant-monitor-swarm-IaC && bash deploy-observability.sh' || {
        echo -e "${RED}⚠️  Observability stack deployment had issues, but continuing...${NC}"
    }
    echo -e "${GREEN}✓ Observability stack deployed${NC}"
    echo ""
    
    # Wait for Prometheus to be ready before deploying autoscaler
    ...
    # Deploy autoscaler
    echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}Phase 3: Autoscaler Deployment${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo ""
    
    echo -e "${YELLOW}→ Deploying HPA-equivalent autoscaler...${NC}"
    ssh -i ~/.ssh/docker-swarm-key ubuntu@${MANAGER_IP} 'cd ~/plant-monitor-swarm-IaC && bash deploy-autoscaler.sh' || {
        echo -e "${RED}⚠️  Autoscaler deployment had issues, but continuing...${NC}"
    }
    ...
    echo "  🤖 Autoscaler deployed and monitoring Kafka consumer lag"
    echo "  📊 View autoscaler logs:"
    echo "     ssh -i ~/.ssh/docker-swarm-key ubuntu@${MANAGER_IP} 'docker service logs -f autoscaler_simple-autoscaler'"
    echo ""
    echo -e "${BLUE}CA3 Evidence Collection:${NC}"
    echo "  📸 Import Grafana Dashboard:"
    echo "     python3 plant-monitor-swarm-IaC/import-dashboard.py"
    echo ""
    echo "  🧪 Run Autoscaling Demo:"
    echo "     ssh -i ~/.ssh/docker-swarm-key ubuntu@${MANAGER_IP} 'cd ~/plant-monitor-swarm-IaC && bash add-sensors.sh 10'"
    echo "     # Wait 2-3 minutes, observe processor scaling 1→2→3"
    echo "     ssh -i ~/.ssh/docker-swarm-key ubuntu@${MANAGER_IP} 'cd ~/plant-monitor-swarm-IaC && bash add-sensors.sh 2'"
    echo "     # Wait 2-3 minutes, observe processor scaling 3→2→1"
    echo ""
    echo "  📹 Evidence Collection Script:"
    echo "     cd plant-monitor-swarm-IaC && bash collect-evidence.sh"
    echo ""
