...
resource "aws_security_group" "swarm_manager_sg" {
  ...
  # CA4: WireGuard VPN for edge-to-cloud connectivity
  ingress {
    from_port   = 51820
    to_port     = 51820
    protocol    = "udp"
  ...
  # CA4: Kafka external listener for VPN clients ONLY
  ingress {
    from_port   = 9092
    to_port     = 9092
    protocol    = "tcp"
...
resource "aws_instance" "swarm_manager" {
  ...
  vpc_security_group_ids = [aws_security_group.swarm_manager_sg.id]
...
resource "aws_instance" "swarm_workers" {
  ...
  vpc_security_group_ids = [aws_security_group.swarm_worker_sg.id]
